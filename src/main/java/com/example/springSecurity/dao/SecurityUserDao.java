...
public interface SecurityUserDao {
	...
	@Insert("insert into securityuser values(#{uid}, #{pwd}, #{uname}, #{email}, default, default,"
			+ " #{picture}, #{provider}, default)")
	void insertSecurityUser(SecurityUser securityUser);
	
	@Update("update securityuser set pwd=#{pwd}, uname=#{uname}, uname=#{uname}, email=#{email},"
